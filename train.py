...
def train_seg_aff(batch_size, epochs, lr_base, lr_power, weight_decay,
                  ...
                  resume_training=False, class_weight=None, dataset='VOC_AFF',
                  loss_fn=softmax_sparse_crossentropy_ignoring_last_label,
                  metrics={"seg":sparse_accuracy_ignoring_last_label, "aff":sparse_accuracy_ignoring_last_label},
                  loss_shape=None,
                  label_suffix='.png',
                  data_suffix='.jpg',
                  ignore_label=255,
    ...
    model.compile(loss={'seg':loss_fn,'aff':loss_fn}, loss_weights = {'seg':0.5, 'aff':0.5},
                  optimizer=optimizer,
                  metrics=metrics)
    if resume_training:
    ...
    if K.backend() == 'tensorflow':
        TB_dir = os.path.join(save_path, f"{log_dir}/{time_str}")
        tensorboard = TensorBoard(log_dir=TB_dir, histogram_freq=0, write_graph=True)
        callbacks.append(tensorboard)
    # ################### checkpoint saver#######################
    checkpoint = ModelCheckpoint(filepath=os.path.join(save_path, f'checkpoint_weights_{time_str}.hdf5'), save_weights_only=True)#.{epoch:d}
    callbacks.append(checkpoint)
    # set data generator and train
    train_datagen = SegAffDataGenerator(zoom_range=[0.5, 2.0],
