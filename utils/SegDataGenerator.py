...
def pair_center_crop_segaff(x, y1, y2, center_crop_size, data_format, **kwargs):
    if data_format == 'channels_first':
        centerh, centerw = x.shape[1] // 2, x.shape[2] // 2
    elif data_format == 'channels_last':
        centerh, centerw = x.shape[0] // 2, x.shape[1] // 2
    ...
    if data_format == 'channels_first':
        return x[:, h_start:h_end, w_start:w_end], \
               y1[:, h_start:h_end, w_start:w_end], \
               y2[:, h_start:h_end, w_start:w_end]
    elif data_format == 'channels_last':
        return x[h_start:h_end, w_start:w_end, :], \
               y1[h_start:h_end, w_start:w_end, :], \
               y2[h_start:h_end, w_start:w_end, :]

def pair_center_crop(x, y1, center_crop_size, data_format, **kwargs):
    if data_format == 'channels_first':
        centerh, centerw = x.shape[1] // 2, x.shape[2] // 2
    elif data_format == 'channels_last':
        centerh, centerw = x.shape[0] // 2, x.shape[1] // 2
...
def pair_random_crop_segaff(x, y1, y2, random_crop_size, data_format, sync_seed=None, **kwargs):
    np.random.seed(sync_seed)
    if data_format == 'channels_first':
        h, w = x.shape[1], x.shape[2]
    elif data_format == 'channels_last':
    ...
    w_start, w_end = offsetw, offsetw + random_crop_size[1]
    if data_format == 'channels_first':
        return x[:, h_start:h_end, w_start:w_end], y1[:, h_start:h_end, h_start:h_end], y2[:, h_start:h_end, h_start:h_end]
    elif data_format == 'channels_last':
        return x[h_start:h_end, w_start:w_end, :], y1[h_start:h_end, w_start:w_end, :], y2[h_start:h_end, w_start:w_end, :]
...
class SegAffDirectoryIterator(Iterator):
    '''
    Users need to ensure that all files exist.
    Label images should be png images where pixel values represents class number.
    ...
    def __init__(self, file_path, seg_data_generator,
        ...
        # build lists for data files and label files
        self.data_files = []
        self.aff_label_files = []
        self.semantic_label_files = []
        fp = open(file_path)
        lines = fp.readlines()
        fp.close()
        self.nb_sample = len(lines)
        for line in lines:
            line = line.strip('\n')
            self.data_files.append(line + data_suffix)
            self.aff_label_files.append(line + label_suffix)
            self.semantic_label_files.append(line + label_suffix)
        super(SegAffDirectoryIterator, self).__init__(
            self.nb_sample, batch_size, shuffle, seed)
    ...
    def _get_batches_of_transformed_samples(self, index_array):
        ...
        for i, j in enumerate(index_array):
            data_file = self.data_files[j]
            aff_label_file = self.aff_label_files[j]
            semantic_label_file = self.semantic_label_files[j]
            img_file_format = 'img'
            img = load_img(os.path.join(self.data_dir, data_file),
                           grayscale=grayscale, target_size=None)
            aff_label_filepath = os.path.join(self.aff_label_dir, aff_label_file)
            semantic_label_filepath = os.path.join(self.semantic_label_dir, semantic_label_file)

            if self.label_file_format == 'npy':
                y1 = np.load(aff_label_filepath)
                y2 = np.load(semantic_label_filepath)
            else:
                aff_label = Image.open(aff_label_filepath)
                semantic_label = Image.open(semantic_label_filepath)
                if self.save_to_dir and self.palette is None:
                    self.palette = aff_label.palette
            ...
            if self.target_size:
                if self.crop_mode != 'none':
                    ...
                    if self.label_file_format is not 'npy':
                        y1 = img_to_array(
                            aff_label, data_format=self.data_format).astype(int)
                        y2 = img_to_array(
                            semantic_label, data_format=self.data_format).astype(int)
                    img_w, img_h = img.size
                    if self.pad_size:
                        pad_w = max(self.pad_size[1] - img_w, 0)
                        pad_h = max(self.pad_size[0] - img_h, 0)
                    ...
                    if self.data_format == 'channels_first':
                        x = np.lib.pad(x, ((0, 0), (pad_h // 2, pad_h - pad_h // 2), (pad_w // 2, pad_w - pad_w // 2)), 'constant', constant_values=0.)
                        y1 = np.lib.pad(y1, ((0, 0), (pad_h // 2, pad_h - pad_h // 2), (pad_w // 2, pad_w - pad_w // 2)),
                                       'constant', constant_values=self.label_cval)
                        y2 = np.lib.pad(y2, ((0, 0), (pad_h // 2, pad_h - pad_h // 2), (pad_w // 2, pad_w - pad_w // 2)),
                                        'constant', constant_values=self.label_cval)
                    elif self.data_format == 'channels_last':
                        x = np.lib.pad(x, ((pad_h // 2, pad_h - pad_h // 2), (pad_w // 2, pad_w - pad_w // 2), (0, 0)), 'constant', constant_values=0.)
                        y1 = np.lib.pad(y1, ((pad_h // 2, pad_h - pad_h // 2), (pad_w // 2, pad_w - pad_w // 2), (0, 0)), 'constant', constant_values=self.label_cval)
                        y2 = np.lib.pad(y2, ((pad_h // 2, pad_h - pad_h // 2), (pad_w // 2, pad_w - pad_w // 2), (0, 0)), 'constant', constant_values=self.label_cval)
                else:
                    x = img_to_array(img.resize((self.target_size[1], self.target_size[0]),
                                                Image.BILINEAR),
                                     data_format=self.data_format)
                    if self.label_file_format is not 'npy':
                        y1 = img_to_array(aff_label.resize((self.target_size[1], self.target_size[
                            0]), Image.NEAREST), data_format=self.data_format).astype(int)
                        y2 = img_to_array(semantic_label.resize((self.target_size[1], self.target_size[
                            0]), Image.NEAREST), data_format=self.data_format).astype(int)
                    else:
                        print('ERROR: resize not implemented for aff_label npy file')
            ...
            if self.target_size is None:
                ...
                if self.loss_shape is not None:
                    ...
                    batch_y1 = np.zeros((current_batch_size,) + y1.shape)
                    batch_y2 = np.zeros((current_batch_size,) + y2.shape)

            x, y1, y2 = self.seg_data_generator.random_transform(x, y1, y2)
            x = self.seg_data_generator.standardize(x)
            ...
            if self.loss_shape is not None:
                ...
                y2 = np.reshape(y2, self.loss_shape)
        ...
        if self.save_to_dir:
            for i in range(current_batch_size):
                ...
                semantic_label.palette = self.palette
                # TODO(ahundt) fix index=i, a hacky workaround since current_index + i is no long available
                fname = '{prefix}_{index}_{hash}'.format(prefix=self.save_prefix,
                                                         index=i,
                                                         hash=np.random.randint(1e4))
                img.save(os.path.join(self.save_to_dir, 'img_' +
                                      fname + '.{format}'.format(format=self.save_format)))
                aff_label.save(os.path.join(self.save_to_dir,
                                        'aff_label_' + fname + '.png'))
                semantic_label.save(os.path.join(self.save_to_dir,
                                            'semantic_label_' + fname + '.png'))
        # return
        batch_x = preprocess_input(batch_x)
        if self.class_mode == 'sparse':
            return batch_x, {'aff':batch_y1, 'seg':batch_y2}
        else:
            return batch_x


class SegAffDataGenerator(object):
    ...
    def flow_from_directory(self, file_path, data_dir, data_suffix,
        ...
        if self.crop_mode == 'random' or self.crop_mode == 'center':
            target_size = self.crop_size
        return SegAffDirectoryIterator(
            file_path, self,
            data_dir=data_dir, data_suffix=data_suffix,
    ...
    def random_transform(self, x, y1, y2):
        # x is a single image, so it doesn't have image number at index 0
        img_row_index = self.row_index - 1
        img_col_index = self.col_index - 1
        img_channel_index = self.channel_index - 1
        ...
        assert x.shape[img_row_index] == y1.shape[img_row_index] and x.shape[img_col_index] == y1.shape[
            img_col_index], 'DATA ERROR: Different shape of data and aff label!\ndata shape: %s, label shape: %s' % (str(x.shape), str(y1.shape))
        assert x.shape[img_row_index] == y2.shape[img_row_index] and x.shape[img_col_index] == y2.shape[
            img_col_index], 'DATA ERROR: Different shape of data and semantic label!\ndata shape: %s, label shape: %s' % (str(x.shape), str(y2.shape))
        ...
        x = apply_transform(x, transform_matrix, img_channel_index,
                            fill_mode=self.fill_mode, cval=self.cval)
        y1 = apply_transform(y1, transform_matrix, img_channel_index,
                             fill_mode='constant', cval=self.label_cval)
        y2 = apply_transform(y2, transform_matrix, img_channel_index,
                             fill_mode='constant', cval=self.label_cval)
        ...
        if self.horizontal_flip:
            if np.random.random() < 0.5:
                x = flip_axis(x, img_col_index)
                y1 = flip_axis(y1, img_col_index)
                y2 = flip_axis(y2, img_col_index)

        if self.vertical_flip:
            if np.random.random() < 0.5:
                x = flip_axis(x, img_row_index)
                y1 = flip_axis(y1, img_row_index)
                y2 = flip_axis(y2, img_row_index)

        if self.crop_mode == 'center':
            x, y1, y2 = pair_center_crop_segaff(x, y1, y2, self.crop_size, self.data_format)
        elif self.crop_mode == 'random':
            x, y1, y2 = pair_random_crop_segaff(x, y1, y2, self.crop_size, self.data_format)
